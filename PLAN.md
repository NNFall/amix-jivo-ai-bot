...
- Production verification: final server suite -> `156 passed`; both services loaded Antigravity primary with the bounded retry budget and Sol fallback; a live HTTP 429 incident completed twice through Sol, including a real `search_products` tool call; audit attribution, health/readiness and fresh journals passed.
...
## Обновление 2026-07-20 - естественный разговорный стиль

- Статус: completed and deployed.
- Цель: сделать ответы менее шаблонными и более похожими на обычную переписку с менеджером, не добавляя словари фраз, keyword-routing или новые функции.
- Граница изменения: только общие правила системного промпта и тест, который фиксирует вариативность речи без механических разговорных частиц и фамильярности.
- Проверка: целевые pytest-тесты и 2-3 длинных диалога через настоящий `gemini-3.1-flash-lite` на VPS с отчётом по функциям, времени, токенам и стоимости.
- Результат:
  - независимый агент подтвердил, что естественность нужно получать через простой синтаксис, вариативность и отсутствие ритуальных фраз, а не через обязательные слова-паразиты;
  - целевые тесты -> `27 passed` локально и на изолированном checkout VPS;
  - финальный live-прогон -> `3/3` диалога и `18/18` ходов, `PASS`;
  - 23 обращения к Gemini, 86 348 токенов, 28,532 секунды provider-latency, около 2,70 рубля;
  - отчёты: `outputs/amix-human-live-0d6779d.md` и `outputs/amix-human-live-0d6779d.json`.
  - production обновлён, целевые серверные тесты прошли, оба сервиса активны, внутренний и внешний `/health` возвращают `{"status":"ok"}`.

## Обновление 2026-07-20 - буквальные человекоподобные диалоги

- Статус: completed and deployed.
- Исправлена ошибка предыдущего отчёта: в чат больше не выводится пересказ вида «бот показал итог» вместо фактической реплики модели.
- Промпт теперь прямо требует в большинстве ответов использовать разные естественные разговорные связки и частицы, а незавершённый заказ всегда продолжать одним ясным следующим вопросом.
- Если клиент называет только общую категорию товара, модель сначала уточняет вариант и количество, а не запускает широкий поиск.
- Поиск описательных названий стал независим от порядка слов, переданных моделью; товар находится по одинаковому набору значимых слов.
- Проверки:
  - локальный и серверный полный pytest -> `136 passed`;
  - три новых реальных диалога -> 20 клиентских ходов, все буквальные ответы и функции сохранены;
  - формальный сценарный итог `18/20`: два несовпадения относятся к заранее ожидаемому порядку сбора полей и падежу имени в summary, а не к пропущенному вопросу или неверной функции;
  - 24 обращения к Gemini, 93 966 токенов, 31,114 секунды provider-latency, около 2,99 рубля.
- Отчёты: `outputs/amix-human-conversation-live-351e1c3.md` и `outputs/amix-human-conversation-live-351e1c3.json`.
- Production обновлён до `351e1c3`; оба сервиса активны, внутренний и внешний `/health` возвращают `{"status":"ok"}`, новых ошибок в журнале нет.

## Обновление 2026-08-04 - ротация ключа Gemini и увеличенные retry

- Статус: completed and deployed.
- Рабочий `GOOGLE_AI_API_KEY` заменён в существующем защищённом окружении VPS; секрет не записан в репозиторий, тесты или отчёты.
- Для бесплатного тарифа Gemini retry увеличен до 6 попыток с общим окном 420 секунд; пауза после HTTP 429 оставлена 65 секунд.
...
## Обновление 2026-08-16 - production-переход на Kaigo Sol

- Статус: completed and deployed.
- Переключить LLM-слой на `gpt-5.6-sol` с `reasoning_effort=low`, сохранив Gemini-конфигурацию для быстрого отката.
- Не добавлять backend-определение намерений по словам. Sol получает полный системный промпт, всю хронологию и описания ровно двух существующих функций.
- Реализовать строгий текстовый JSON-протокол, который преобразуется в существующие `ToolCall` для `search_products` и `handoff_to_manager`.
- Валидировать имя функции и аргументы; неизвестные функции не выполнять. При нарушении формата сделать один корректирующий повтор, затем вернуть безопасную ошибку провайдера.
- Проверить реальные сценарии из последних Jivo-диалогов, затем задеплоить на VPS, переключить `.env`, перезапустить сервисы и проверить health, логи, функции и Jivo-ready поведение.
- Отдельно зафиксировать статистику Gemini за 14 и 30 дней и найденные ошибки.
- Реализован text-only адаптер Sol со строгим ответом `assistant` или `tool_call`, полной историей, валидацией двух функций, корректирующим повтором и обработкой временных ошибок.
- Для соблюдения лимита Kaigo последовательные запросы разнесены минимум на 6,2 секунды; Gemini-настройки сохранены в `.env` для отката.
- Live-проверка полного заказа и отсутствующего товара: 13 клиентских ходов, 17 успешных обращений Sol, обе функции и финальная сверка отработали; 4 узкие текстовые проверки не совпали из-за другого корректного порядка вопросов и формулировок.
- Live-проверка сценариев из последних Jivo-диалогов после уточнения общего приоритета функций: `4/4` сценария и `4/4` хода, `PASS`.
