...
- Следующий шаг:
  - после проверки пользователем задеплоить на VPS и выдать URL `/admin`;
  - перед реальным использованием заменить `ADMIN_PASSWORD=change-me` на сильный пароль в `.env` сервера.
