...
LLM-слой расширен под `kie.ai` с моделью `gpt-5-2`, локальные и серверные live-тесты API прошли. VPS уже синхронизирован с KIE-интеграцией; для живого демо теперь не хватает только Telegram token и товарного XML.
...
- Подтянуть Telegram bot token и реальный XML AMIX, затем запустить demo service на готовом KIE-backed сервере.
...
Заполнить `/root/amix/.env` Telegram token, импортировать XML AMIX и запустить `amix-telegram-demo.service` на уже настроенном KIE-провайдере.
