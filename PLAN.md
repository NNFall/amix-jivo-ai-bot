...
- Статус: локальная реализация завершена, live-eval и независимое ревью выполняются.
- `search_products` принимает отдельное требуемое количество для каждой позиции и сохраняет исходный порядок запросов.
- Gemini получает только результат достаточности количества, без точного свободного остатка; точное значение остаётся внутри backend для расчёта.
- Лимит трёх проверок считается отдельно по коду товара; явный новый артикул больше не подменяется прошлым товаром из контекста.
- Автоматический backend-prelookup по умолчанию выключен; основной товарный поиск инициирует Gemini.
