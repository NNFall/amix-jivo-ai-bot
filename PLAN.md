...
Код Telegram demo уже развернут на VPS в `/root/amix`, Python-окружение и systemd unit подготовлены. Для живого запуска демо не хватает только рабочих секретов и товарной XML-базы.
...
Получить Telegram bot token, OpenAI API key и XML-файл AMIX, затем заполнить `/root/amix/.env`, импортировать данные и запустить `amix-telegram-demo.service`.
