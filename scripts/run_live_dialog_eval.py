...
DEFAULT_SCENARIOS = [
    ...
    LiveScenario("L-023", "Дубль без лишней таблицы", "есть мп 28ск", "Несколько позиций, не выдавать таблицу, попросить код или цену."),
    LiveScenario(
        "L-024",
    ...
    LiveScenario(
        "L-032",
        "Память первого артикула",
        "а какие еще есть про те которые я сперва спросил",
        "Вспомнить МП 28ск и варианты по нему, не отвечать общим приветствием.",
        history=("есть мп 28ск", "198 которая стоит", "Проверьте 14.023пр. и XYZ-999"),
    ),
    LiveScenario(
        "L-033",
        "Не понял после уточнения",
        "не понял",
        "Не здороваться, кратко пояснить последнюю найденную позицию МП 28ск.",
        history=("есть мп 28ск", "198 которая стоит"),
    ),
    LiveScenario("L-034", "Цена без корпоративной", "сколько стоит 1108035?", "Показать розничную цену и не показывать корпоративную без запроса."),
    LiveScenario("L-035", "Дубль без ссылки и фото", "есть мп 28ск", "Попросить код или цену, не просить ссылку или фото."),
]
...
def _content_flags(scenario: LiveScenario, answer: str, payload: dict[str, Any]) -> list[str]:
    ...
    if scenario.case_id in {"L-023", "L-035"}:
        if "ссыл" in lower or "фото" in lower:
            flags.append("asks_link_or_photo_for_duplicate_article")

    if scenario.case_id == "L-034":
        if "корпоратив" in lower:
            flags.append("corporate_price_without_request")

    if scenario.case_id in {"L-032", "L-033"}:
        if "подскажите, что нужно посмотреть" in lower or "добрый день" in lower:
            flags.append("lost_context_generic_greeting")
        if "мп 28" not in lower and "26169" not in lower:
            flags.append("previous_product_not_recalled")
