...
| Production model must actually enter the order flow | If an explicit order turn returns plain text without a tool call, Gemini is retried once with `update_order_draft` selected as the required tool; only the tool-backed response reaches the customer. | `core/assistant_service.py`, `test_explicit_order_retries_with_required_order_tool_when_model_returns_only_text` |
