...
При намерении оформить заказ модель обновляет структурированный черновик через `update_order_draft`. Бот собирает товары и количество, желаемый срок, получение, оплату и контакты; для безналичного счёта дополнительно нужны данные организации или ИП. Свободное описание товара допустимо. После итогового резюме `order_creation` разрешается только при явном подтверждении клиента.
