...
# 2026-08-16 - подготовка production-перехода на Kaigo Sol

- Перед изменением production выполнен read-only аудит истории `llm_calls` и последних реальных Jivo-диалогов.
- Статистика Gemini:
  - за 14 дней: 107 успешных обращений, 434 863 токена, средняя provider-latency 2 224 мс;
  - за 30 дней: 257 успешных обращений, 1 043 555 токенов, средняя provider-latency 1 704 мс;
  - найдено пять HTTP 403 за 2-4 августа; более поздних ошибок Gemini и HTTP 429 в сохранённых вызовах нет;
  - найденные в journal строки HTTP 429 относятся к Telegram polling, а не к Gemini.
- Реализован адаптер text-only Kaigo API для `gpt-5.6-sol` / `low`:
  - полный системный промпт и полная хронологическая история передаются без backend-классификации текста;
  - модель возвращает строгий JSON: готовый ответ или вызов одной из двух функций;
  - разрешены только `search_products` и `handoff_to_manager`, имена и обязательные аргументы валидируются;
  - при неверном формате выполняется один корректирующий запрос;
  - HTTP 429, временные серверные ошибки, таймауты и сетевые сбои проходят через ограниченный retry;
  - последовательные запросы ограничены интервалом 6,2 секунды, чтобы один многораундовый ответ не превышал документированный лимит Kaigo;
  - usage и latency сохраняются существующим журналом LLM-вызовов.
- Live history-eval больше не привязан жёстко к Gemini и использует провайдера из окружения.
- TDD: исходные тесты нового провайдера сначала падали; отдельный тест HTTP 429 обнаружил неправильное имя общей retry-функции, после чего дефект исправлен.
- Локальные проверки до deploy:
  - шесть Kaigo protocol/retry тестов -> пройдены;
  - связанные `test_llm_client`, `test_history_order_eval`, `test_model_driven_assistant`, `test_dialog_service` -> `64 passed`;
  - `compileall`, `git diff --check` и поиск секретов -> успешно.
- Прямой запрос к Kaigo с VPS -> HTTP 200, `gpt-5.6-sol` ответил корректно; ключ хранится только в защищённом `.env` VPS.
- Первый live-прогон двух длинных сценариев:
  - 13 клиентских ходов, 17 успешных LLM-вызовов, 203 444 токена, 101 058 мс provider-latency;
  - поиск товаров, повторная проверка исправленного количества, сбор данных, итог, подтверждение и handoff выполнены корректно;
  - формальный итог `9/13` ходов из-за узких текстовых ожиданий: другой порядок корректных вопросов и словоформа «обе» вместо «оба»; ошибок провайдера и функций не было.
- Проверка четырёх формулировок из последних Jivo-диалогов выявила, что Sol сначала заменял обязательный поиск/передачу уточняющим вопросом и сделал неподтверждённый вывод об услуге ремонта.
- Протокол усилен общим приоритетом действий без backend keyword-routing: новый товар требует поиска, подбор и небезопасный ответ требуют handoff; текст вместо обязательной функции считается нарушением.
- Повторная live-проверка последних Jivo-сценариев -> `4/4` сценария и `4/4` хода, `PASS`:
  - адрес отвечен из справки без функции;
  - ремонт передан менеджеру без выдуманного ответа;
  - помощь с выбором передана менеджеру;
  - новое название товара вызвало поиск, после отсутствующего результата запрошен код или цена.
  - 5 успешных обращений Sol, 58 114 токенов, 26 114 мс provider-latency.
- Отчёты сохранены локально:
  - `outputs/kaigo-sol-live-2026-08-16.md` / `.json`;
  - `outputs/kaigo-recent-jivo-live-2026-08-16.md` / `.json`.
- Production deploy:
  - ветка отправлена в GitHub и VPS обновлён fast-forward до `7b4c5a9`;
  - `.env` переключён на `kaigo` / `gpt-5.6-sol` / `low`, Gemini-конфигурация не удалена;
  - серверный полный pytest -> `146 passed`;
  - `amix-api.service` и `amix-telegram-demo.service` -> `active`;
  - `http://127.0.0.1:8010/health` и `https://amix.cifresh.ru/health` -> HTTP 200, `{"status":"ok"}`;
  - process environment подтверждает активные provider/model/effort/interval без вывода секрета;
  - production `OpenAIService` вернул адрес текстом и `search_products` для кода 770; свежий journal ошибок не содержит.
- Независимый критический review обнаружил два высоких граничных дефекта:
  - запрос на подбор с новым артикулом одновременно подпадал под обязательный поиск и handoff;
  - `KAIGO_RETRY_TOTAL_TIMEOUT_SECONDS=300` не ограничивал один HTTP read timeout 900 секунд и следующий backoff.
- Оба замечания исправлены:
  - правила функций упорядочены, handoff имеет приоритет над поиском; живой запрос «Подберите аналог для 14.023пр» вернул `handoff_to_manager` с причиной `compatibility_or_analogs`;
  - timeout каждого запроса и задержка retry ограничиваются оставшимся общим бюджетом.
- После исправлений локальный и серверный полный pytest повторно -> `146 passed`; production-код обновлён до `1446a8a`, сервисы перезапущены.
- Первый внешний health сразу после restart кратко вернул HTTP 502; после ожидания готовности внутренний и внешний health стабильно вернули HTTP 200 и `{"status":"ok"}`. Свежих ошибок в journal нет.

# 2026-08-17 - переход на Antigravity Gemini 3.7 Flash

- Изучены предоставленные Antigravity `llms.txt` и OpenAPI 3.1; подтверждены выбор модели/reasoning, `messages`, native function calling, двухэтапный `tool_call -> tool result -> final response`, режим встроенных инструментов и HTTP 429 при превышении пяти одновременных запросов.
- Прямыми диагностическими запросами подтверждено:
  - обычный текстовый ответ и нативный `search_products` возвращаются через HTTP 200;
  - endpoint отклоняет сохранённый исторический `assistant.tool_calls` объект с HTTP 400;
  - исходное сообщение клиента и последующий хронологический `role=tool` результат принимаются и дают корректный финальный ответ.
- Реализован отдельный Antigravity-провайдер:
  - системный промпт передаётся отдельно, полная user/assistant/tool история сохраняется;
  - исторический транспортный объект assistant tool call пропускается только из-за ограничения endpoint, при этом полная исходная история остаётся в SQLite, а результаты функций остаются в хронологическом месте;
  - объявляются только `search_products` и `handoff_to_manager`;
  - встроенные Antigravity-инструменты принудительно отключены независимо от значения окружения;
  - текст, функции, usage, latency и retryable errors преобразуются в существующий `LLMTurnResult`;
  - HTTP 429, 500, 502, 503, 504, timeout и сетевые ошибки проходят через ограниченный retry;
  - `output_tokens`, включающие reasoning, разделяются на completion/thinking и сохраняются вместе с provider/model/latency в SQLite.
- На одинаковой выборке из 17 клиентских ходов сравнены модели:
  - `gemini-3.7-flash-low`: 23 обращения, 71 528 мс provider-latency, 556 080 токенов;
  - `gemini-3.7-flash-medium`: 23 обращения, 90 896 мс, 562 913 токенов;
  - `gemini-3.7-flash-high`: 23 обращения, 153 726 мс, 565 804 токена;
  - production Sol: 23 обращения, 131 946 мс, 280 648 токенов.
- Выбрана `gemini-3.7-flash-low`: medium/high не дали функционального выигрыша и работали медленнее; low стабильнее Sol соблюдала AMIX-сценарии, хотя provider-reported token usage примерно вдвое выше из-за накладных расходов Antigravity.
- Полный live history-order прогон low после доработки промпта:
  - `9/9` сценариев и `41/41` клиентских ходов;
  - `54/54` успешных обращений провайдера;
  - 181 569 мс суммарной provider-latency и 1 228 822 provider-reported токена;
  - повтор нестабильного invoice-сценария: `3/3` сценария, `18/18` ходов;
  - post-fix live smoke: `1/1` сценарий, `2/2` хода, реальный вызов `search_products` и корректная отмена заказа без лишнего handoff.
- Исправлен общий prompt-инвариант: при оплате по счёту бот запрашивает обязательный ИНН до перехода к финальной сверке.
- Независимый критический review обнаружил retry для HTTP 500 и неверную семантику output/thinking tokens; оба замечания исправлены тестами. Повторный review не нашёл P0-P2 замечаний.
- Локальная финальная проверка:
  - `python -m pytest -q` -> `153 passed`;
  - Antigravity-focused tests -> `5 passed`;
  - SQLite usage persistence test -> `1 passed`;
  - `compileall`, `git diff --check` и поиск секретов -> успешно.
- Production deploy:
  - коммиты `87490b5` и `043c24b` отправлены в GitHub, VPS обновлён fast-forward до `043c24b`;
  - серверный полный pytest до переключения -> `153 passed`;
  - текущий `.env` сохранён в защищённой rollback-копии, новый ключ передан отдельным временным файлом с режимом `600`, `.env` обновлён атомарно, временный файл удалён;
  - production переключён на `antigravity` / `gemini-3.7-flash-low` / `low` / `native_tools=none`; прежняя Kaigo/Gemini-конфигурация оставлена для быстрого отката;
  - изолированный серверный live smoke до restart -> `1/1` сценарий и `2/2` хода, без отправки сообщений в Jivo;
  - `amix-api.service` и `amix-telegram-demo.service` -> `active`;
  - process environment обоих сервисов подтверждает новый provider/model/effort/native-tools без вывода ключа;
  - `http://127.0.0.1:8010/health`, `/ready` и `https://amix.cifresh.ru/health` -> HTTP 200;
  - свежие журналы после перезапуска не содержат warning/error, автоматический XML-импорт завершился успешно;
  - production SQLite WAL/SHM и ранее сохранённые сравнительные отчёты не изменялись и не удалялись.

# 2026-08-17 - production-инцидент Antigravity и откат на Sol

- Получено сообщение, что Jivo-бот отвечает защитным текстом о задержке автоматической проверки.
- Проверка production показала:
  - `amix-api.service` и `amix-telegram-demo.service` активны;
  - внутренние `/health` и `/ready` возвращают HTTP 200;
  - XML-импорт выполняется успешно;
  - Jivo-события сохраняются и обрабатываются.
- В journal обнаружены серии из трёх HTTP 502 от `https://kaigo.space/antigravity-api/v1/respond` для реальных обращений в 08:25, 09:23, 13:33 и 14:44 UTC.
- В SQLite за день сохранено пять `LLMCall` со статусом `http_502`; из-за supersession быстрых сообщений клиентам были отправлены три fallback-ответа:
  - вопрос об адресе магазина;
  - технический вопрос по мойке;
  - начало оформления заказа.
- Ключ и конфигурация проверены без вывода секрета:
  - hash локального и серверного ключа совпадает;
  - минимальный запрос с тем же ключом локально вернул HTTP 200;
  - с VPS тот же запрос сначала завершился timeout, затем без изменений вернул HTTP 200 за 2,13 с;
  - это подтверждает плавающую доступность внешнего Antigravity-контура, а не ошибку ключа, модели, payload, Jivo или AMIX-кода.
- Резервный Kaigo Sol перед переключением проверен прямым запросом: HTTP 200 за 5,87 с.
- Выполнено безопасное восстановление:
  - создана защищённая резервная копия текущего `.env`;
  - атомарно изменён только `LLM_PROVIDER` с `antigravity` на `kaigo`;
  - Antigravity-ключ и настройки сохранены для последующего возврата;
  - оба production-сервиса перезапущены и подтвердили `kaigo` / `gpt-5.6-sol` / `low` в process environment.
- Изолированный live smoke после переключения:
  - `1/1` сценарий, `2/2` клиентских хода;
  - реальный `search_products` выполнен;
  - отмена заказа и смена темы обработаны без лишней функции;
  - verdict `PASS`.
- Финальная проверка: оба сервиса активны, публичный health возвращает `{"status":"ok"}`, свежих warning/error в journal нет.
- Код не менялся: корневая неисправность находится во внешнем Antigravity endpoint. Production оставлен на Sol до подтверждения стабильности Antigravity или реализации отдельного автоматического failover.

# 2026-08-17 - автоматический failover Antigravity на Sol

- Зафиксирован контракт отказоустойчивости без изменения диалоговой архитектуры: Antigravity остаётся основным провайдером, выполняет до трёх попыток с существующими паузами, после чего тот же ход с полной историей и теми же двумя функциями отправляется в `kaigo` / `gpt-5.6-sol`.
- Безопасный клиентский текст о задержке теперь остаётся только последней защитой, если не ответили оба провайдера.
- `LLMTurnResult` дополнен фактическими provider/model и отметкой исходного провайдера; SQLite сохраняет успешное восстановление как `provider=kaigo`, `model=gpt-5.6-sol`, `status=ok_fallback_from_antigravity`.
- Общая latency резервного хода включает ожидание основного провайдера и вызов Sol.
- После server-side проверки старой конфигурации ограничен зависающий primary-контур: connect/read одной попытки установлены в 5/15 секунд, общий retry-бюджет снижен с 300 до 90 секунд. Даже три полных connect+read и паузы 7/14 секунд занимают не более 81 секунды, поэтому Sol не блокируется старым пятиминутным ожиданием.
- Добавлен failing-first тест: три HTTP 502 Antigravity, затем успешный Sol; отдельно проверены порядок четырёх HTTP-запросов и фактические provider/model/status.
- Независимый read-only review не нашёл блокирующих дефектов, но обнаружил неверную подпись резервного запроса в JSON-аудите. Замечание воспроизведено отдельным падающим assert и исправлено передачей фактического провайдера в аудит и расчёт стоимости.
- Локальная проверка:
  - focused failover/persistence -> `2 passed`;
  - связанные LLM/assistant tests -> `42 passed`;
  - полный `python -m pytest -q` -> `155 passed`;
...
# 2026-08-28 - повторный сбой обоих LLM-провайдеров и восстановление через direct Gemini

- Получено сообщение, что Jivo-бот снова отвечает аварийным текстом вместо ответа клиенту.
- Read-only проверка production установила точную границу сбоя:
  - оба systemd-сервиса работали без перезапусков;
  - внутренние `/health` и `/ready`, а также публичный `/health` возвращали HTTP 200;
  - автоматический XML-импорт продолжал успешно обновлять товары;
  - Jivo-событие от 2026-08-27 было принято, сохранено и помечено обработанным;
  - три попытки Antigravity завершились read timeout, после чего три попытки резервного `gpt-5.6-sol` завершились HTTP 502 `Codex did not return a usable answer`;
  - клиенту закономерно ушёл последний защитный текст о задержке автоматической проверки.
- По статистике с 17 августа подтверждено, что сбой внешний и повторяющийся: Antigravity периодически отвечал, но часто уходил в timeout/429; Sol восстанавливал часть запросов, однако 24, 25 и 27 августа сам начал возвращать HTTP 502.
- С VPS выполнены отдельные контрольные запросы без вывода ключей:
  - Antigravity -> HTTP 502;
  - Kaigo Sol -> HTTP 502;
  - direct Google AI Studio / `gemini-3.1-flash-lite` -> успешный `OK` примерно за 1 секунду.
- Перед переключением direct Gemini отдельно подтвердил native function calling: модель вызвала `search_products` для артикула и количества, приняла tool-result и сформировала клиентский ответ.
- Production восстановлен конфигурационно без изменения диалоговой архитектуры:
  - создана защищённая rollback-копия `.env`;
  - `LLM_PROVIDER` атомарно переключён с `antigravity` на `google_ai_studio`;
  - оба сервиса перезапущены и подтвердили `google_ai_studio` / `gemini-3.1-flash-lite` в process environment;
  - внутренние health/readiness и публичный health -> HTTP 200.
- Изолированный end-to-end smoke выполнен на временной копии production SQLite без отправки сообщений в Jivo:
  - повторено последнее проблемное обращение по OrgaTray;
  - бот сначала запросил требуемое количество;
  - после уточнения вызвал реальный `search_products`, обработал результат и предложил безопасное уточнение артикула/кода;
  - три model-turn завершились со статусом `ok` и latency 1 240-1 593 мс;
  - объявлены только две разрешённые функции: `search_products` и `handoff_to_manager`;
  - временная база удалена после проверки, production SQLite и Jivo-история не изменялись.
- Финальная проверка после переключения:
  - локальный `python -m pytest -q` -> `156 passed`;
  - VPS `/root/amix/.venv/bin/python -m pytest -q` -> `156 passed`;
  - локальный и VPS `compileall`, `git diff --check`, а также проверка двух разрешённых функций -> успешно;
  - оба сервиса -> `active`, внутренние `/health` и `/ready`, публичный `/health` -> HTTP 200;
  - в journal после перезапуска нет warning/error;
  - untracked SQLite WAL/SHM и сравнительные отчёты сохранены.
- Отдельно зафиксирован инфраструктурный риск, не связанный с этим инцидентом: корневой раздел VPS занят на 91%, свободно 2.7 GB. Очистку диска нужно провести отдельной контролируемой задачей.

# 2026-08-28 - возврат быстрого Antigravity с финальным direct Gemini failover

- Перед изменением production повторно проверены все три внешних контура с VPS без вывода ключей:
  - Antigravity `gemini-3.7-flash-low` возвращал HTTP 429/502;
  - Codex `gpt-5.6-luna` и `gpt-5.6-sol` возвращали HTTP 502 `codex_unavailable`;
  - direct Google AI Studio `gemini-3.1-flash-lite` успешно ответил примерно за 1.24 секунды.
- Дополнительно сравнены доступные быстрые Antigravity-модели на реальном вызове `search_products`:
  - `gemini-3.5-flash-low` вернул корректный вызов функции за 1.61-2.41 секунды;
  - `gemini-3.6-flash-low` вернул тот же вызов за 22.46-43.91 секунды;
  - финальный запрос после результата функции на обеих моделях завершался HTTP 502 внешнего Antigravity provider.
- Выбран `gemini-3.5-flash-low`: это фактически самая быстрая доступная Flash-low модель в текущем окружении VPS.
- Реализована трёхступенчатая цепочка без изменения диалоговой архитектуры и без новых функций:
  - основной Antigravity с существующими тремя ограниченными попытками;
  - резервный Codex `gpt-5.6-sol` с той же полной историей и теми же функциями;
  - финальный direct Google AI Studio, если Codex также не вернул текст или функцию.
- Исправлена атрибуция общего OpenAI-compatible аудита: записи direct Google и KIE теперь используют фактический provider, а не активный primary provider; расчёт стоимости также получает фактический provider.
- TDD-проверки:
  - новый provider-chain тест сначала падал, потому что direct Google не вызывался после ошибки Codex, затем прошёл после минимальной реализации;
  - отдельная audit-проверка сначала обнаружила неверный `provider=antigravity`, затем прошла после исправления атрибуции;
  - default-model тест сначала зафиксировал старый `gemini-3.7-flash-low`, затем прошёл после выбора `gemini-3.5-flash-low`.
- Локальная проверка до deployment:
...
# 2026-08-28 - проверка качества production-ответов

- На временной копии production SQLite без отправки сообщений в Jivo проведены четыре живых сценария: адрес, наличие с уточнением количества, технический подбор и сбор заказа до итоговой сверки.
- Техническая часть прошла корректно:
  - товарные факты получены только через `search_products`;
  - точный остаток клиенту не раскрывался;
  - техническая совместимость передана через реальный `handoff_to_manager`;
  - заказ собран до полного итога без преждевременной передачи менеджеру;
  - неизвестных функций и выдуманных товарных фактов не обнаружено.
- Ручная оценка выявила два повторяющихся отклонения качества, которые автоматические flags не считали ошибкой:
  - после самостоятельной проверки наличия бот без просьбы клиента начинал спрашивать получение и город;
  - на короткий общий вопрос бот добавлял сведения, которых клиент не запрашивал.
- Причина находится в недостаточно приоритетной границе между консультацией и уже начатым заказом, а не в поиске товара или кодовой маршрутизации.
- Через TDD добавлен обобщённый prompt-инвариант без частных примеров: самостоятельная консультация не запускает сбор заказа, ответ остаётся в пределах текущего намерения, а следующий вопрос задаётся только при реальной необходимости.
- Локальная проверка после правки:
  - новый тест сначала падал на отсутствующем правиле, после изменения промпта прошёл;
  - связанные prompt/LLM tests -> `44 passed`;
  - полный `python -m pytest -q` -> `158 passed`;
  - `compileall`, `git diff --check` и проверка ровно двух функций -> успешно.

# 2026-08-28 - повторная ручная проверка границы консультации и заказа

- На временной копии production SQLite повторены реальные сценарии адреса, наличия с количеством и полного сбора заказа с подтверждением; сообщения в Jivo не отправлялись.
- Полный заказ отработал правильно: бот запросил товары, проверил оба количества через `search_products`, собрал получение, срок, оплату, имя, телефон и ИНН, показал полный итог и только после подтверждения вызвал `handoff_to_manager` с причиной `order_creation`.
- При этом повторно подтверждены два отклонения вне заказа:
  - ответ на вопрос об адресе добавил режим работы;
  - после ответа о доступности двух штук бот сам перешёл к вопросу о самовывозе или доставке.
- Трассировка показала, что функции и backend работают корректно; проблема вызвана тем, что общий prompt-запрет недостаточно жёстко отделял обычную консультацию от активного сбора заказа.
- Добавлены failing-first prompt-инварианты, после чего правило усилено обобщённо, без keyword-router и частных сценарных костылей:
  - консультация является режимом по умолчанию до явного намерения клиента оформить заказ;
  - названное количество само по себе не запускает заказ;
  - завершённый ответ по наличию не продолжается вопросами об условиях заказа;
  - справочный ответ остаётся внутри запрошенной группы сведений.
- Локальная проверка: новые тесты сначала дали `2 failed`, после изменения промпта -> `2 passed`; полный `python -m pytest -q` -> `159 passed`; `compileall` и `git diff --check` -> успешно.
- Изменение опубликовано коммитом `26d4ada`, VPS обновлён fast-forward; серверный полный `pytest` -> `159 passed`, оба systemd-сервиса перезапущены и активны.
- Финальный изолированный live-ретест после deployment подтвердил:
  - `14.023пр есть?` -> бот запросил количество;
  - `две штуки` -> вызван `search_products`, бот только подтвердил наличие нужного количества и не начал спрашивать доставку или оплату;
  - явное `Мне нужно оформить заказ` по-прежнему включило сбор заказа;
  - после двух кодов и количеств вызван `search_products`, оба количества подтверждены, затем бот перешёл к следующему шагу заказа;
  - вопрос об адресе получил правильный адрес без функций; модель дополнительно назвала правильный график работы. Это оставлено как допустимый естественный контекст, чтобы не превращать справочные ответы в жёсткий скрипт.
- Внешние провайдеры во время финального ретеста оставались нестабильны: Antigravity отвечал 429/502, Codex - HTTP 400; финальные ответы успешно сформировал direct `gemini-3.1-flash-lite` через существующий failover.
