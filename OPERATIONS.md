...
- После server-side проверки старой конфигурации ограничен зависающий primary-контур: read timeout одной попытки снижен с 900 до 20 секунд, общий retry-бюджет с 300 до 90 секунд. Это оставляет время на три попытки с паузами и не задерживает переход на Sol на пять минут.
